...
import { EventPlannerPDF } from './EventPlannerPDF';
...
export default function EventPlannerOutput({ planContent: initialContent, onReset }: Props) {
  ...
  const handleExportWord = async () => {
    ...
    exportToWord(content, 'etkinlik-plani', 'Etkinlik ve Kutlama Programı');
  };
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
      <div className="bg-slate-50 dark:bg-slate-800/50 p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-4 print:hidden">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <PDFDownloadLink
            document={<EventPlannerPDF markdown={content} />}
            fileName="Etkinlik_Plani.pdf"
