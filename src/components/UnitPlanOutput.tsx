...
import { UnitPlanPDF } from './UnitPlanPDF';
import MarkdownEditor from './MarkdownEditor';

interface UnitPlanOutputProps {
  markdown: string;
  onReset: () => void;
}

export default function UnitPlanOutput({ markdown: initialMarkdown, onReset }: UnitPlanOutputProps) {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const blob = await pdf(<UnitPlanPDF markdown={markdown} />).toBlob();
      
      // Create download link and trigger click
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `Unite_Plani.pdf`;
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            onClick={async () => {
              ...
              exportToWord(markdown, 'unite-plani', 'Ünite Planı');
            }}
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-medium shadow-sm disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isGeneratingPDF ? (
      ...
      <div className="p-8 md:p-12 max-w-4xl mx-auto bg-white dark:bg-slate-900 print:p-0">
        {isEditing ? (
          ...
          <div className="prose prose-slate dark:prose-invert prose-emerald max-w-none prose-headings:font-bold prose-h1:text-3xl prose-h1:text-center prose-h1:mb-8 prose-h1:border-b-2 prose-h1:border-slate-800 dark:prose-h1:border-slate-200 prose-h1:pb-6 prose-h2:text-2xl prose-h2:text-emerald-700 dark:prose-h2:text-emerald-400 prose-h2:mt-10 prose-h2:border-b prose-h2:border-slate-200 dark:prose-h2:border-slate-700 prose-h2:pb-2 prose-h3:text-xl prose-h3:text-slate-800 dark:prose-h3:text-slate-100 prose-h3:mt-8 prose-li:my-1">
