...
export default function BEPOutput({ bepContent: initialContent, onReset }: Props) {
  const [bepContent, setBepContent] = useState(initialContent);
  ...
  const handleExportWord = async () => {
    ...
    exportToWord(bepContent, 'bep-taslagi', 'Bireyselleştirilmiş Eğitim Programı (BEP)');
  };
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
      <div className="bg-slate-50 dark:bg-slate-800/50 p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-4 print:hidden">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <PDFDownloadLink
            document={<BEPPDF markdown={bepContent} />}
            fileName="BEP_Taslagi.pdf"
      ...
      <div className="p-8 md:p-12 max-w-4xl mx-auto bg-white dark:bg-slate-900 print:p-0">
        {isEditing ? (
          <MarkdownEditor value={bepContent} onChange={(val) => setBepContent(val || '')} />
        ) : (
