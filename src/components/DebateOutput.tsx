...
import { DebatePDF } from './DebatePDF';
...
export default function DebateOutput({ planContent: initialPlanContent, onReset }: Props) {
  const [content, setContent] = useState(initialPlanContent);
  ...
  const handleExportWord = async () => {
    ...
    exportToWord(content, 'tartisma-plani', 'Sınıf İçi Tartışma / Münazara Planı');
  };
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
      <div className="bg-slate-50 dark:bg-slate-800/50 p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-4 print:hidden">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <PDFDownloadLink
            document={<DebatePDF markdown={content} />}
            fileName="Tartisma_Plani.pdf"
